...
/*   Updated: 2024/07/30 11:47:51 by zibnoukh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../minishell.h"

void use_pipe(t_box *box, int fd[2])
{
...
void child_procees(t_box* box, int prev_fd, char **files, int i, int fd[2])
{
...
void more_then___(t_box *box)
{
    ...
    while (box->node->command && i <  n(files) + 1)
    {
        ...
        if (pid == -1)
        {
            ...
            child_procees(box, prev_fd, files, i,fd);
        if (prev_fd != -1)
            close(prev_fd);
        if (box->node->command->next)
        {
